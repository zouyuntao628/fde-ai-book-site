...
const bookChapters = [
  {
    ...
    summary: "开篇解释企业 AI 为什么经常停在 Demo、试点和领导参观阶段，无法进入真实业务流程。重点不是证明 AI 有用，而是说明企业需要一种能把模型能力、业务流程、数据权限、用户采用和风险治理连在一起的新型落地能力。",
    sections: [
  ...
  },
  {
    ...
    part: "上篇：为什么需要 FDE",
    title: "什么是 FDE",
    summary: "本章集中定义 FDE：它不是一个更会写代码的售前，也不是一个更懂业务的顾问，而是对企业 AI 落地结果负责的现场工程角色。它的价值在于把模糊需求变成可运行、可评估、可运营的业务系统。",
    sections: [
      ["FDE 的定义：把 AI 放进真实业务流程的人", "FDE 的核心任务是让 AI 能在业务现场被正确使用，而不是只完成技术演示。它同时关注业务目标、系统能力、数据条件、用户行为和组织约束。"],
      ["FDE 与售前、顾问、工程师、产品经理的区别", "FDE 会参与销售前后的关键判断，但不以签单为终点；会参与工程实现，但不只对代码负责；会参与产品反馈，但不只写需求文档。"],
      ["FDE 做什么，不做什么", "FDE 做场景判断、方案设计、MVP 闭环、生产化推进和复盘沉淀；不替代客户组织责任，也不把所有定制需求都变成项目承诺。"],
      ["FDE 对结果负责到什么程度", "FDE 应对可验证的业务闭环负责，包括用户是否使用、输出是否可信、流程是否跑通、指标是否改善。"],
      ["边界不清时项目为什么会失败", "当 FDE 被当作售后、外包或万能协调员时，项目会失去优先级、范围和复盘机制，最后变成无法复制的个案。"]
    ],
    sources: ["02", "04"]
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      ["场景评分：价值、数据、验证、风险、采用", "FDE 可以用统一评分框架比较场景，避免只选择最炫或最高层关注的题目。"],
  ...
  },
  {
    ...
    part: "中篇：如何把 AI 落到生产",
    title: "如何评估 AI 项目的价值",
    summary: "本章把 AI 项目从感觉好用拉回到可衡量。评估既包括技术质量，也包括业务结果、安全风险和长期 ROI。",
    sections: [
      ["企业 AI 不能靠感觉评估", "领导觉得惊艳、用户觉得方便、模型回答流畅，都不足以证明项目成功。评估需要可重复的数据和业务指标。"],
  ...
  },
  {
    ...
    sections: [
      ...
      ["医药与医疗：人审优先、证据优先、合规优先", "医疗和医药场景要围绕证据链、合规边界和专业人审设计，避免把 AI 包装成未经验证的自动决策者。"],
  ...
  },
  {
    ...
    sections: [
      ["FDE 团队不是高级外包团队", "FDE 的目标不是无限满足客户定制，而是通过现场项目发现可复用问题、推动真实落地并反哺产品。"],
  ...
  },
  {
    ...
    summary: "最后一章收束到个人和未来。FDE 既是一条职业路径，也是一种会被 AI 工具持续增强的组织能力。未来不是 FDE 消失，而是 FDE 的工作方式被 Agent、自动化评估和行业知识系统重塑。",
    sections: [
  ...
  container.innerHTML = `
    ...
    <section>
      ...
      <div class="section-list">
        ${chapter.sections.map(([title, description], sectionIndex) => `
